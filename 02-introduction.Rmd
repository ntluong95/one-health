...
```{r, message = FALSE, warning=FALSE}
lb_df |>
  dplyr::filter(sex == "Male", age >= 5, breed %in% c("Hereford", "Angus", "Simental"))
```
...
**Filter row with `.query()`** Filtering rows from a Pandas dataframe can be done in different ways.
...
lb_df.loc[(lb_df["sex"] == "Male") & 
          (lb_df["age"] >= 5) & 
          (lb_df["breed"].isin(["Hereford", "Angus", "Simental"]))]

lb_df >> filter(_.sex == "Male", _.age >= 5, _.breed.isin(["Hereford", "Angus", "Simental"]))
...
lb_df >> arrange(-_.case) #"-" means sort in descending
```
...
lb_df.loc[:, "case":"temp"]
lb_df.iloc[:, 0:12]
lb_df.loc[:, lb_df.columns.str.endswith("se", "mp")]

lb_df1.loc[:, lb_df1.columns.str.contains('|'.join(['se', 'mp']))]

search_values = 

lb_df1 = lb_df
lb_df1.rename(columns = {'case':'a1'}, inplace = True) 
lb_df1.rename(columns = {'age':'a2'}, inplace = True) 

lb_df1 >> select(_.contains('|'.join(['se', 'mp'])))
```
