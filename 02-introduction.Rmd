...
To **label for variables** in dataset, you can use function `var_labels` in package `sjlabelled`. Noted that the operation `|>` means *forwarding*. It is R native piping, similar to `%>%` if you know `magrittr` package
...
lb_df <- raw_df |> 
  sjlabelled::var_labels(
    ...
    breed = "breed",
    sex = "sex",
    attd = "attitude of calf",
    dehy = "% dehydration",
    eye = "uveitis/hypopyon clinically evident",
    jnts = "swollen joints clinically evident",
    post = "posture of caIf",
    pulse = "pulse rate",
    resp = "respiratory rate",
    temp = "rectal temperature",
    umb = "swollen umbilicus clinically evident",
    sepsis = "sepsis")
```
...
![](docs/images/Screenshot%202021-01-30%20135810.png)

</center>

If you want to see the label of varible, i.e "case", "age" and "umb", you can create a dictionary.

```{r, message = FALSE, warning=FALSE}
# create data dictionary ----
dictionary <- labelled::look_for(lb_df, details = TRUE)
dictionary %>% 
  dplyr::filter(variable %in% c("case", "age", "umb")) %>% 
...
**Create new variable** To *label for value of variables* in dataset, you can use function
