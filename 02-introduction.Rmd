...
lb_df1 = raw_df \
    >> mutate(
          sex = _.apply(lambda x: sex_dict.get(x["sex"], "Unknown"), axis = 1),
          breed = _.apply(lambda x: breed_dict.get(x["breed"], "Unknown"), axis = 1),
          attd = _.apply(lambda x: attd_dict.get(x["attd"], "Unknown"), axis = 1),
          eye = _.apply(lambda x: eye_dict.get(x["eye"], "Unknown"), axis = 1),
          post = _.apply(lambda x: post_dict.get(x["post"], "Unknown"), axis = 1),
          umb = _.apply(lambda x: umb_dict.get(x["umb"], "Unknown"), axis = 1),
          sepsis = _.apply(lambda x: sepsis_dict.get(x["sepsis"], "Unknown"), axis = 1)
          )
print(lb_df1)
```

Same version with Pandas function
